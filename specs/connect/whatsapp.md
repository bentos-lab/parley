...
- **connect**: Run `parley connect whatsapp` to print a QR code and pair your WhatsApp account. The pairing data stays under `~/.bentos/parley/connect/whatsapp/session.json`. If a session already exists, the command prompts for confirmation before removing the old store and creating a fresh pairing.
...
- **History persistence**: The listener saves only `/parley` command messages and `[parley]` replies to the JSON cache at `~/.bentos/parley/connect/whatsapp/whatsapp.history.json`, reloads it on startup, and keeps the most recent 10 entries per chat so that history survives restarts.
- **Behavior**:
